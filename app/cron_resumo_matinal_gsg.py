#!/usr/bin/env python3
"""
cron_resumo_matinal_gsg.py - Resumo financeiro diario GSG
Envia todo dia as 08h: inadimplencia + vencendo hoje + comercial
"""
...
def buscar_dados():
    try:
        ...
        r_inad = ixc_select_one("""
            SELECT COUNT(DISTINCT cc.id) as qtd, SUM(f.valor_aberto) as valor
...
def montar_msg():
    ...
    r_inad, r_hoje, r_com, top_vend = buscar_dados()

    cabecalho = f"<b>{saudacao} -- GSG {data_fmt}</b>"
    ...
    if r_hoje:
        ...
        linhas.append(f"  Vencendo hoje: <b>{qtd_h} titulos</b> | {fmt_brl(val_h)}")
    linhas.append("")
    ...
    if top_vend:
        ...
        linhas.append(f"  Lider do mes: <b>{nome}</b> ({int(top_vend['total'])} ativ.)")
    linhas.append("")
    linhas.append(f"Bom trabalho, equipe! -- {saudacao}!")
...
if __name__ == "__main__":
    ...
    if msg:
        notificar(msg, chat)
        print(f"Resumo enviado para {chat}")
