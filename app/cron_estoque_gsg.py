...
DB_EST   = str(BASE_DIR / "app" / "gsg_estoque.db")
...
def buscar_produtos_alerta():
    ...
    for r in rows:
        ...
        qtd_sug = min(500, max(20, int(consumo * 30 * 2 - saldo))) if consumo > 0 else 0
        cat     = r["categoria"]
        ...
        if (saldo <= 0 and consumo > 0) or (consumo > 0 and dias < DIAS_CRITICO):
