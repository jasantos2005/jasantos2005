...
def buscar_usuario_no_banco(login, senha):
    try:
        ...
        cur.execute(query, (login, senha_hash))
        user = cur.fetchone()
        conn.close()
        return user
...
async def rota_portal(request: Request):
    user = request.session.get("user")
    if not user or user["nivel"] != 99:
        return RedirectResponse(url="/dashboard/home", status_code=303)
    usuarios = admin_service.listar_usuarios_com_permissoes()
    logs = admin_service.listar_logs()
    return templates.TemplateResponse("portal/index.html", {"request": request, "session": request.session, "usuarios": usuarios, "logs": logs})


@app.post("/admin/usuarios/salvar")
async def salvar_usuario(request: Request,
    usuario_id: str = Form(default=""),
    nome: str = Form(...),
    login: str = Form(...),
    senha: str = Form(default=""),
    setor: str = Form(default=""),
    nivel: int = Form(default=1),
    permissao: str = Form(default="")):
    user = request.session.get("user")
    if not user or user["nivel"] != 99:
        return RedirectResponse(url="/", status_code=303)
    import hashlib
    senha_hash = hashlib.sha256(senha.encode()).hexdigest() if senha and senha.strip() else "..."
    print(f"DEBUG salvar: id={usuario_id} nome={nome} login={login} setor={setor} nivel={nivel} permissao={permissao} senha_vazia={not senha.strip()}")
    admin_service.cadastrar_usuario(nome, login, senha_hash, nivel, setor, permissao)
