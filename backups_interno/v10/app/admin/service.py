...
class AdminService:
    ...
    def registrar_acao(self, usuario, acao, tipo="info"):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO auditoria (usuario, acao, tipo) VALUES (?, ?, ?)",
            (usuario, acao, tipo)
        )
        conn.commit()
        conn.close()
    ...
    def cadastrar_usuario(self, nome, login, senha, nivel, setor, permissao_dash=""):
        ...
        if exists:
            ...
            self.registrar_acao("Sistema", f"Usuário {login} atualizado", "warning")
        else:
            cursor.execute("""
            ...
            self.registrar_acao("Sistema", f"Novo usuário {login} criado", "success")
